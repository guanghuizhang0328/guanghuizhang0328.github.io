...
%%--------------------------Factors for P2 for increased trials and all trials --------------------------------

%%Load factors for withinsubject PCA
X = importdata([pathName, 'P2_withinsubject_PCA_factor_selected.mat']);
X_mean(1,:) = squeeze(mean(X,2));
X_std(1,:)  = squeeze(std(X,0,2))/sqrt(17);
X1 = importdata([pathName,'P2_withinsubject_PCA_factor_selected_all_trials.mat']);
X_mean(1,34) = squeeze(mean(X1,2));
X_std(1,34)  = squeeze(std(X1,0,2))/sqrt(17);

%%%Load factors for trial-averaged group PCA
X_mean(2,1:33) = importdata([pathName,'P2_Group_averaged_PCA_factor_selected.mat']);
X_std(2,:)  = 0;
X_mean(2,34) = 2;
X_std(2,34)  = 0;

%%Load factors for single-trial-based group PCA
X_mean(3,1:33) = importdata([pathName,'P2_Group_single_PCA_factor_selected.mat']);
X_std(3,:)  = 0;
X_mean(3,34) = 2;
X_std(3,34)  = 0;
...
title(['P2:Numbers of selected factors for increased trials and all trials'])
...
X_mean = [];
X_std =[];
X = importdata([pathName, 'N2_withinsubject_PCA_factor_selected.mat']);
X_mean(1,:) = squeeze(mean(X,2));
X_std(1,:)  = squeeze(std(X,0,2))/sqrt(17);
X1 = importdata([pathName,'N2_withinsubject_PCA_factor_selected_all_trials.mat']);
X_mean(1,34) = squeeze(mean(X1,2));
X_std(1,34)  = squeeze(std(X1,0,2))/sqrt(17);

%%Load factors for trial-averaged group PCA
X_mean(2,1:33) = importdata([pathName,'N2_Group_averaged_PCA_factor_selected.mat']);
X_std(2,:)  = 0;
X_mean(2,34) = 4;
X_std(2,34)  = 0;
%%Load factors for single-trial-based group PCA
X_mean(3,1:33) = importdata([pathName,'N2_Group_single_PCA_factor_selected.mat']);
X_std(3,:)  = 0;
X_mean(3,34) = 4;
X_std(3,34)  = 0;
...
title(['N2:Numbers of selected factors for increased trials and all trials'])
legend({'Trial-averaged group-PCA','Single-trial-based group PCA','Winthin-subject PCA'});
legend('boxoff');
set(gca,'xtick',[0:35]);
set(gca,'ytick',[1:1:5]);
