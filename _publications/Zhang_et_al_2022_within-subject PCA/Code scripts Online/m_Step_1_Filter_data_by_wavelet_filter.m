% % %Script #1
%%%  Filtering averaged ERP or sing-trial EEG data after preprocessed
...
route  =strcat('1_wavelet_filter',filesep);
mkdir(route);

for Numofsub  =1:length(subName)
    pathName_data = strcat(Subject_file_Path,filesep,'Sub',num2str(subName(Numofsub)),filesep);
    for Numofsti  =1:length(stiName)
        ...
        EEG = pop_loadbv(pathName_data,fn);
