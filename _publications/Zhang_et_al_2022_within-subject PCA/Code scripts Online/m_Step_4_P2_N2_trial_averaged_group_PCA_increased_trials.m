...
%%% To extract P2 and N2 from trial-averaged ERP of all participants for increased trials
...
Factor_P2 = importdata([pathName_factor,'P2_Group_averaged_PCA_factor_order_increased_trials.mat']);
Factor_N2 = importdata([pathName_factor,'N2_Group_averaged_PCA_factor_order_increased_trials.mat']);
...
for Numofcomponent = 1:2% 1.P2; 2.N2
    ...
    for Numoftrials = 10:42
        if Numofcomponent ==1
            route1 = char(strcat('2_PCA for increased trials\P2\2_Trial averaged group PCA',filesep,'Trial_',num2str(Numoftrials),filesep));
        else
            route1 = char(strcat('2_PCA for increased trials\N2\2_Trial averaged group PCA',filesep,'Trial_',num2str(Numoftrials),filesep));
        end
        mkdir(route1);
        X = [];
        for Numofsub = 1:length(subName)
            ...
            for Numofsti = 1:stiNum
                ...
                X(:,:,Numofsti,Numofsub) = squeeze(mean(data(:,:,[1:Numoftrials]),3));
            end
        end
        ...
        if Flag_plot_component == 1
            ...
            pause;
            prompt = {['Reset the orders of the factor of interest for trial-averaged group PCA ',32,'with',32,num2str(Numoftrials),'trials:']};
            dlg_title = 'Input';
            num_lines = 1;
