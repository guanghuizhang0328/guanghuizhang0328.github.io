% % %m_Step_4_P2_N2_trial_averaged_group_PCA_increased_trials
%%% To extract P2 and N2 from single-trial EEG of all participants for increased trials
...
Factor_P2 = importdata([pathName_factor,'P2_Group_single_PCA_factor_order_increased_trials.mat']);
Factor_N2 = importdata([pathName_factor,'N2_Group_single_PCA_factor_order_increased_trials.mat']);
...
for Numofcomponent = 1:2% 1.P2; 2.N2
    ...
    for Numoftrials = 10:42
        if Numofcomponent ==1
            route1 = char(strcat('2_PCA for increased trials\P2\3_Single trial based group PCA',filesep,'Trial_',num2str(Numoftrials),filesep));
        else
            route1 = char(strcat('2_PCA for increased trials\N2\3_Single trial based group PCA',filesep,'Trial_',num2str(Numoftrials),filesep));
        end
        mkdir(route1);
        X = [];
        for Numofsub = 1:length(subName)
            ...
            for Numofsti = 1:stiNum
                ...
                X(:,:,Numofsti,Numofsub,:) = squeeze(data(:,:,[1:Numoftrials]));
            end
        end
        [NumChans,NumSamps,NumSti,NumSubs,NumTrials] = size(X);
        X = permute(X,[2 1 3 4 5]);
        %% grouping data into a matrix
        X= reshape(X,NumSamps,NumChans*NumSti*NumSubs*NumTrials);
        ...
        topo  =[];
        %%
        topo = reshape(Y',R,NumChans,NumSti,NumSubs,NumTrials);
        topo = squeeze(mean(topo,5));
        topo_similarity_AV =[];
        ...
        if Flag_plot_component == 1
            msgbox(['If you want to reset orders of selected factors for trials#',num2str(subNNumoftrials),',Please press any key!!!'])
            pause;
            prompt = {['Reset the orders of the factor of interest for single-trial-based group PCA ',32,'with',32,num2str(Numoftrials),'trials:']};
            dlg_title = 'Input';
            num_lines = 1;
            def = {num2str(Selectedcomponents_K{Numoftrials-9})};
            answer = inputdlg(prompt,dlg_title,num_lines,def);
            selectedComponentNumbers = str2num(answer{1});
        else
            selectedComponentNumbers = Selectedcomponents_K{Numoftrials-9};%% the orders of  selected components associated with N2
        end
        ...
        temp = reshape(E,NumSamps,NumChans,NumSti,NumSubs,NumTrials);
        D = permute(temp,[2 1 3 4 5]);
        D = squeeze(mean(D,5));
