...
for idf=1:dfs(curFac)
    if FacRem==0
        ...
        for iNextFac=curFac+1:length(dfs)-FacRem+1                    
            [M, MList]=RLoop( FacRem,iNextFac,nextRow,curMNum+ ToAdd, nextList, M,MList,dfs,alphabet );           
            ToAdd=ToAdd+nchoosek(length(dfs)-iNextFac,FacRem-1); % this will apply to the NEXT loop iteration   
        end
    end
end
