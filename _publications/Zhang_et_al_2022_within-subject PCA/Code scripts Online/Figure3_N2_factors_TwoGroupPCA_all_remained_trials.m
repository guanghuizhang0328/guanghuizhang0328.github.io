% % %Figure3_N2_factors_TwoGroupPCA_all_remained_trials
%%% To plot the temporal and spatial factors related to N2 for two group
...
Subject_Name = [1:4,7:10,13,14:16,18:22,24:26];%%Orders for all participants that used to extract N2.
...
save('X_WF_all_trials_N2.mat','X','-v7.3');
...
Selected_k = [2 5 9 26];%%Orders for the indetified factors for P2%
...
fileNameNew = ['PCA_average_group_N2_R_',num2str(R),'_K_',num2str(Selected_k)];
...
subplot(rowNum,columnNum,[1:12],'align');
set(gca,'fontsize',12);
hold on;
...
title('Factors for N2 for trial-averaged group PCA');
...
fprintf(['N2:Correlations between factors for trial-averaged group PCA.\n']);
fprintf( [repmat('_',1,70) '\n']);
fprintf( '%s %8s %6s %6s %6s\n', hdr{:});
fprintf( '\n #5 %12g %9g %9g \n', [cdata(2,1),[],[]]);
fprintf( '\n #9 %12g %7g %7g \n', [cdata(3,1),cdata(3,2),[]]);
fprintf( '\n #36 %11g %7g %6g \n', [cdata(4,1),cdata(4,2),cdata(4,3)]);
fprintf( [repmat('_',1,70) '\n']);
...
Selected_k = [3 10 12 16];%%Identified factors
...
fileNameNew = ['PCA_single_trial_group_N2_R_',num2str(R),'_K_',num2str(Selected_k)];
...
ylabel('Magnitude');
set(gca,'ydir','reverse');
title('Factors for N2 for single-trial-based group PCA');
for Numoffactor = 1:numel(Selected_k)
    LegendName{Numoffactor} = char(strcat('Factor',32,num2str(Selected_k(Numoffactor)),'- Explained:',num2str(roundn(ratio_sigle_keep(Selected_k(Numoffactor)),-2)),'%-Latency:',num2str(peak_T(Selected_k(Numoffactor))),'ms'));
end
legend(LegendName,'location','best');
...
fprintf( ['\n\n\n\n\n'])
fprintf( [repmat('_',1,70) '\n']);
fprintf(['N2:Correlations between factors for single-trial-based group PCA.\n']);
fprintf( [repmat('_',1,70) '\n']);
fprintf( '%s %8s %6s %6s %6s \n', hdr{:});
fprintf( '\n #5 %12g %9g %9g \n', [cdata(2,1),[],[]]);
fprintf( '\n #9 %12g %6g %7g \n', [cdata(3,1),cdata(3,2),[]]);
