% % %Figure4_Tab3_P2_wave_topo_four_methods_all_remained_trials
%%% Plot grand averaged waveforms and toographies for all conditions for P2(all remained trials)
...
load chanlocs;
%%
D(:,:,:,:,1) = importdata('X_WF_all_trials_P2.mat');
D(:,:,:,:,4) = importdata('PCA_withinsubject_P2.mat');
D(:,:,:,:,2) = importdata('PCA_average_group_P2_R_40_K_4  15.mat');
D(:,:,:,:,3) = importdata('PCA_single_trial_group_P2_R_40_K_7  10.mat');
...
for Numofmethod = 1:NumMethod
    ...
    legend(Sti_names);
    legend('boxoff');
    tN = strcat('P2:Grand averaged waves for',32,MethodName{Numofmethod});
    title(tN);
    %%Topographies
    for sti = 1:NumSti
        if sti<4
        ...
        topoplot(squeeze(Topo_av(:,sti,Numofmethod)),chanlocs,'maplimits',[-3,3]);
        hold on;
        tN = [Sti_names{sti}];
...
for Numofmethod = 1:4
    ...
    fprintf(['P2:Statistical analysis results for',32,MethodName{Numofmethod} ,'.\n']);
